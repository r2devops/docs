...
1. If you haven't yet a `.gitlab-ci.yml` file in the root on your repository:
   you can create it and use the following list of stages:
...
2. Select the templates you want in the [Marketplace](https://r2devops.io/marketplace) and append them in the
...
3. The templates can be customized 👉 check the [templates
...
<Button href="https://r2devops.io/marketplace">
  <span>See open-source marketplace 🚀</span>
</Button>
...
By default, each templates from the marketplace is a part of on these stages:
...
own custom stage list: you can re-declare yourself the stage of any template from
the marketplace. Follow the [customization section](#templates-customization) to do it.
...
:::info
All the templates from the `r2devops/hub` specify a docker image to be run in a
docker container.
:::

### 🖌 Global

Each templates of the hub can be customized. To do it, you have to include the template as usual and, in addition, override the options you want to customize.
